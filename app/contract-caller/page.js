      ...
      if (Array.isArray(addresses)) {
        ...
        addresses.forEach((addr, i) => {
          if (!isValidEthAddress(addr)) {
            ...
            const fieldName = path || input.name || `Argument ${argIndex + 1}`;
            argErrors.push(
              `${fieldName}[${i}] must be a valid Ethereum address`,
            );
            valid = false;
          }
        });
  ...
  if (type === "tuple" && input.components) {
    ...
    input.components.forEach((component, i) => {
      const componentPath = path
        ? `${path}.${component.name || i}`
        : `${input.name || `Argument ${argIndex + 1}`}.${component.name || i}`;
      if (
      ...
      if (Array.isArray(tupleArray)) {
        ...
        tupleArray.forEach((tuple, i) => {
          const tuplePath = path
            ? `${path}[${i}]`
            : `${input.name || `Argument ${argIndex + 1}`}[${i}]`;
...
function TupleArgInput({
  ...
  return (
    <div
      className={styles.tupleContainer}
...
function ArrayArgInput({
  ...
  const handleAddItem = () => {
    const newItem =
      isBaseTuple && input.components ? input.components.map(() => "") : "";
...
export default function ContractCaller() {
        ...
        {trace.calls && trace.calls.length > 0 && (
          <div className={styles.traceChildren}>
            {trace.calls.map((child, i) =>
              renderCallTraceNode(child, `${depth}-${i}`),
            )}
          </div>
        )}
      </div>
  ...
  const getCombinedSuggestions = () => {
    ...
    const cached = cachedAddresses.map((item) => {
      const bookmark = addressBook.find(
        (b) => b.address.toLowerCase() === item.address.toLowerCase(),
      );
      return {
        ...item,
              ...
              <div className={styles.addressRow}>
                <div className={styles.addressInputWrapper}>
                  ...
                  {showAddressSuggestions &&
                    getCombinedSuggestions().length > 0 && (
                      <div className={styles.addressSuggestions}>
                        {getCombinedSuggestions()
                          .filter((item) => {
                            ...
                            if (!item.isBookmarked && item.chain !== chain)
                              return false;
                            const textMatch =
                              addressFilter === "" ||
                              item.address
                ...
                <label
                  ...
                  title="Use on-chain detection for proxy contracts not recognized by Etherscan (e.g. Safe, EIP-1167 clones)"
                >
                  <input
                    type="checkbox"
                    checked={detectProxy}
                ...
                {abiViewMode === "list" && parsedAbi && (
                  <div className={styles.abiListView}>
                    {(() => {
                      ...
                      if (totalCount === 0) {
                        return (
                          <div className={styles.abiEmptyState}>
                            {abiFilter
                              ? "No matching entries"
                              : "No ABI entries"}
                          </div>
                          ...
                          {entries.functions.length > 0 && (
                            <div className={styles.abiCategory}>
                              <div className={styles.abiCategoryHeader}>
                                <span className={styles.abiCategoryLabel}>
                                  Functions
                                </span>
                                <span className={styles.abiCategoryCount}>
                                  {entries.functions.length}
                                </span>
                              </div>
                              <div className={styles.abiCategoryItems}>
                                {entries.functions.map((item, idx) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <span className={styles.abiItemBadge}>
                                        {isReadOnly(item) ? "R" : "W"}
                                      </span>
                                      <span className={styles.abiItemSignature}>
                                        {formatAbiSignature(item)}
                                      </span>
                                      {abiCopiedItem === itemKey && (
                                        <span className={styles.abiCopiedBadge}>
                                          Copied!
                                        </span>
                                      )}
                                    </div>
            ...
            <div className={styles.tabSection}>
              ...
              {activeTab === "functions" && functions.length > 0 && (
                <>
                    ...
                    <div className={styles.functionSelectWrapper}>
                      {selectedFunction && getSelectedFunction() ? (
                        ...
                        <input
                          ...
                          onBlur={() =>
                            setTimeout(() => setShowFunctionList(false), 200)
                          }
                      ...
                      {showFunctionList && (
                        <div className={styles.functionList}>
                          ...
                          {getFilteredFunctions().length === 0 && (
                            <div className={styles.functionItemEmpty}>
                    ...
                    !isReadOnly(getSelectedFunction()) && (
                      <div className={styles.simOptionsSection}>
                        <div className={styles.simOptionsHeader}>
                          <span className={styles.simOptionsLabel}>
                            Simulation Options
                          </span>
                          <button
                            onClick={() =>
                              setSimOptionsExpanded(!simOptionsExpanded)
                            }
                            ...
                            {!useLocalSimulation && (
                              <>
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setTimestampOverride(e.target.value)
                                  }
                            ...
                            <div className={styles.simOptionsExpanded}>
                              {cheatcodes.deal.enabled && (
                                <div className={styles.cheatcodeExpandedRow}>
                                  <span className={styles.cheatcodeLabel}>
                                    vm.deal:
                                  </span>
                                  <input
                                    type="text"
                  ...
                  {selectedFunction &&
                    getSelectedFunction() &&
                    isPayable(getSelectedFunction()) && (
                      <div className={styles.field}>
                        <label className={styles.label}>
                          ETH Value{" "}
                          <span className={styles.payableBadge}>payable</span>
                        </label>
                  ...
                  {selectedFunction &&
                    getSelectedFunctionInputs().length > 0 && (
                        ...
                        <div className={styles.argsSectionHeader}>
                          ...
                          {isReadOnly(getSelectedFunction()) && (
                            <div className={styles.readBlockInline}>
                              <label className={styles.readBlockLabel}>
                                Block
                              </label>
                              <input
                                type="text"
                      ...
                      <>
                        ...
                        <div className={styles.eventList}>
                          {getFilteredEvents().map((event) => (
                            ...
                            >
                              <input
                                ...
                                onChange={() =>
                                  toggleEventSelection(event.name)
                                }
                              />
                          ...
                          {getFilteredEvents().length === 0 && (
                            <div className={styles.eventItemEmpty}>
                              No matching events
                            </div>
                          )}
                        </div>
                    ...
                    <div className={styles.blockRangeControls}>
                      <label>
                        ...
                        <input
                          ...
                          placeholder={
                            latestBlockCache
                              ? `${Math.max(0, latestBlockCache - 10000)}`
                              : "latest-10k"
                          }
                    ...
                    <div className={styles.paginationControls}>
                      ...
                      <label>
                        ...
                        <select
                          value={logsOffset}
                          onChange={(e) =>
                            setLogsOffset(parseInt(e.target.value))
                          }
                          ...
                          <div className={styles.filterInputWrapper}>
                            ...
                            <span className={styles.filterHelpIcon}>
                              ...
                              <div className={styles.filterHelpPopup}>
                                <div className={styles.filterHelpTitle}>
                                  Filter Syntax
                                </div>
                                <div className={styles.filterHelpRow}>
                                  <span className={styles.filterHelpLabel}>
                                    Fields:
                                  </span>
                                  <code>event</code> <code>args.*</code>{" "}
                                  <code>topic0-3</code> <code>data</code>{" "}
                                  <code>block</code> <code>tx</code>
                                </div>
                                <div className={styles.filterHelpRow}>
                                  <span className={styles.filterHelpLabel}>
                                    Operators:
                                ...
                                <div className={styles.filterHelpExample}>
                                  Example: event = Transfer and args.value &gt;
                                  1000
                                </div>
                              </div>
                            </span>
        ...
        >
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <h3 className={styles.modalTitle}>
              ...
              getBookmarkedAddress(bookmarkAddress || address)
                ? "Edit Bookmark"
                : "Add to Address Book"}
            </h3>
              ...
              {customChains.length > 0 && (
                <div className={styles.addedChainsSection}>
                  <button
                    ...
                    onClick={() =>
                      setAddedChainsCollapsed(!addedChainsCollapsed)
                    }
                  >
                    ...
                    <div className={styles.addedChainsList}>
                      {customChains.map((c) => (
                        <div key={c.id} className={styles.addedChainItem}>
                          ...
                          <span className={styles.addedChainName}>
                            {c.name}
                          </span>
                          <span className={styles.addedChainId}>
                            #{c.chainId}
                          </span>
                          <button
                            onClick={(e) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={styles.chainlistItemInfo}>
                            <span className={styles.chainlistItemName}>
                              {chainData.name}
                            </span>
                          ...
                          {added ? (
                            <span className={styles.chainlistItemAdded}>
                              Added
                            </span>
                          ) : (
                            <button className={styles.addChainItemButton}>
                              + Add
                            </button>
                          )}
                        </div>
