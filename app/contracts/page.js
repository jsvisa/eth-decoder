    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith(ABI_CACHE_PREFIX)) {
        ...
        contracts.push({
          ...
          eventCount:
            cached.abi?.filter((item) => item.type === "event").length || 0,
        });
      }
    }
  } catch (err) {
...
export default function Contracts() {
    ...
    if (searchFilter.trim()) {
      ...
      return (
        ...
        (contract.implContractName &&
          contract.implContractName.toLowerCase().includes(search))
      );
    }
  ...
  const handleDelete = (contract) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to delete the cached ABI for "${name}"?`,
      )
    ) {
      return;
    }
  ...
  const handleDeleteAll = () => {
    if (
      !window.confirm(
        `Are you sure you want to delete all ${contracts.length} cached contract ABIs? This cannot be undone.`,
      )
    ) {
      return;
    }
  ...
  const handleFileSelect = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setSuccess(
          `Imported ${newCount + updatedCount} contracts (${newCount} new${updatedCount > 0 ? `, ${updatedCount} updated` : ""})`,
        );
  ...
  return (
    <main className={styles.main}>
      <div className={styles.container}>
        ...
        {success && success.includes("copied") && (
          <div className={styles.toast}>{success}</div>
        )}
        ...
        {contracts.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <p>
              Contracts will appear here after you fetch their ABIs in the
              Contract Caller.
            </p>
                      ...
                      <div className={styles.chainCell}>
                        ...
                        <div className={styles.chainInfo}>
                          <span className={styles.chainName}>
                            {contract.chainInfo?.name || contract.chain}
                          </span>
                          {contract.chainInfo?.chainId && (
                            <span className={styles.chainId}>
                              ID: {contract.chainInfo.chainId}
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              <code className={styles.csvFormat}>
                chain, address, contractName, implContractName, implAddress,
                isProxy, timestamp, abi
              </code>
              <p className={styles.importNote}>
                The <strong>chain</strong> and <strong>address</strong> columns
                are required. Use <strong>Export CSV</strong> to see the exact
                format.
              </p>
