  ...
  try {
    ...
    if (!tenderlyAccessKey || !tenderlyAccount || !tenderlyProject) {
      return NextResponse.json(
        {
          error:
            "Tenderly API credentials not provided. Please configure your Tenderly settings.",
        },
        { status: 400 },
      );
    }
      ...
      if (stateOverrides.storage && stateOverrides.storage.length > 0) {
        for (const override of stateOverrides.storage) {
          if (override.address && override.slot && override.value) {
            ...
            const slot = override.slot.startsWith("0x")
              ? override.slot
              : "0x" + override.slot;
            const value = override.value.startsWith("0x")
              ? override.value
              : "0x" + override.value;
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.error?.message || `Tenderly API returned ${response.status}`,
      );
    }
    ...
    const extractContractCalls = (trace, logsArray = []) => {
      ...
      if (isContractCall) {
        ...
        const decodedOutputs =
          trace.decoded_output?.map((output) => ({
    ...
    const buildCallTraceTree = (trace, logsArray = []) => {
      ...
      const callLogs = logsArray
        .filter(
          (log) => log.raw?.address?.toLowerCase() === trace.to?.toLowerCase(),
        )
        .map((log) => ({
          name: log.name || "Unknown Event",
          address: log.raw?.address,
