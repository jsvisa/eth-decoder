...
export default function AddressBook() {
  ...
  const handleDelete = (item) => {
    const name =
      item.label || item.contractName || item.address.slice(0, 10) + "...";
    if (
      !window.confirm(
        `Are you sure you want to remove "${name}" from your address book?`,
      )
    ) {
      return;
    }
  ...
  const handleFileSelect = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const updatedCount = importOverwrite
          ? importedEntries.length - newCount
          : 0;

        setShowImportModal(false);
        setSuccess(
          `Imported ${importedEntries.length} entries (${newCount} new${updatedCount > 0 ? `, ${updatedCount} updated` : ""})`,
        );
  ...
  return (
    <main className={styles.main}>
      <div className={styles.container}>
        ...
        {isToast && <div className={styles.toast}>{success}</div>}
        ...
        {addressBook.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <p>
              Add addresses from the Contract Caller or click "Add Address" to
              get started.
            </p>
          </div>
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              <p className={styles.importNote}>
                The <strong>address</strong> column is required. Other columns
                are optional.
              </p>
              <div className={styles.modalField}>
