...
export default function EventDecoder() {
  ...
  useEffect(() => {
    ...
    if (d) setData(d)
    if (t) setTimeout(() => document.querySelector('form')?.requestSubmit(), 100)
  }, [])
  ...
  const saveToHistory = (topicList, dataHex, output) => {
    ...
    const next = [item, ...history.filter(h => h.topics[0] !== topicList[0])].slice(0, MAX_HISTORY_ITEMS)
    setHistory(next)
  ...
  const updateTopic = (i, val) => setTopics(t => t.map((v, idx) => idx === i ? val : v))
  ...
  const handleSubmit = async (e) => {
    ...
    if (!topic0) { setError('Topic0 (event signature hash) is required'); return }
    if (!isValidHex(topic0) || topic0.replace('0x', '').length !== 64) {
      setError('Topic0 must be a 32-byte hex string (64 hex chars)'); return
    }
    for (let i = 1; i < topics.length; i++) {
      if (topics[i].trim() && !isValidHex(topics[i])) {
        setError(`Topic${i} is not valid hex`); return
      }
    }
    if (data.trim() && !isValidHex(data)) { setError('Data must be valid hex'); return }
    ...
    try {
      ...
      if (json.msg === 'not found') throw new Error('Event signature not found in database')
      if (json.msg !== 'ok' || !json.data) throw new Error(json.error || 'Decode failed')
      ...
      <div className={styles.container}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.topicsSection}>
            ...
            {topics.length < 4 && (
              <button type="button" onClick={addTopic} className={styles.addTopicBtn} disabled={loading}>
                + Add topic
              </button>
            )}
          </div>
          ...
          <div className={styles.dataSection}>
            <div className={styles.sectionLabel}>Data <span className={styles.optional}>(optional)</span></div>
            <input
              type="text"
              value={data}
          ...
          <div className={styles.buttonGroup}>
            ...
            <button
              ...
              disabled={loading}
              onClick={() => { setTopics(['']); setData(''); setResult(null); setError(null) }}
            >
              Clear
            </button>
          </div>
        ...
        {result && (
          <div className={styles.result}>
            <div className={styles.resultHeader}>
              <h2 className={styles.eventName}>{result.event || 'Decoded Event'}</h2>
              <div className={styles.resultActions}>
                <button onClick={() => setIsYaml(!isYaml)} className={styles.actionButton} type="button">
                  {isYaml ? 'JSON' : 'YAML'}
                </button>
                <button onClick={handleCopy} className={styles.actionButton} type="button">
                  {copied ? 'Copied!' : 'Copy'}
                </button>
              </div>
            </div>
              ...
              <div className={styles.argsTable}>
                {result.inputs.map((inp, i) => (
                  <div key={i} className={styles.argRow}>
                    <span className={styles.argName}>{inp.name || `arg${i}`}</span>
                    <span className={styles.argType}>{inp.type}</span>
                    {inp.indexed && <span className={styles.argIndexed}>indexed</span>}
            ...
            <pre
              ...
              dangerouslySetInnerHTML={{ __html: isYaml ? getDisplayContent() : syntaxHighlight(result) }}
            />
          </div>
        )}
          ...
          <div className={styles.historySection}>
            <div className={styles.historyHeader}>
              ...
              <div className={styles.historyActions}>
                <button onClick={() => setShowHistory(!showHistory)} className={styles.historyToggle} type="button">
                  {showHistory ? 'Hide' : 'Show'}
                </button>
                <button
                  onClick={() => { if (window.confirm('Clear all history?')) { setHistory([]); localStorage.removeItem(STORAGE_KEY) } }}
                  ...
                  >
                    <div className={styles.historyTop}>
                      <div className={styles.historyInput}>
                        {item.topics[0]?.slice(0, 10)}...{item.topics[0]?.slice(-6)}
                        {item.topics.length > 1 && <span className={styles.historyTopicCount}> +{item.topics.length - 1}</span>}
                      </div>
                      {item.output?.event && (
                        <div className={styles.historyFunc}>{item.output.event}</div>
                      )}
                    </div>
