...
export const exportContractsToCSV = (contractsList) => {
  const headers = ['chain', 'address', 'contractName', 'implContractName', 'implAddress', 'isProxy', 'timestamp', 'abi']
  const rows = contractsList.map(contract => {
    let cached = {}
    try { cached = JSON.parse(localStorage.getItem(contract.key) || '{}') } catch {}
    return [
      contract.chain,
      contract.address,
