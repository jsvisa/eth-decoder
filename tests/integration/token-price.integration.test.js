...
const hasTokenPriceKey = !!process.env.INTEGRATION_ENABLE_TOKEN_PRICE;
...
describe("GET /api/token-price (integration)", () => {
  ...
  it.skipIf(!hasTokenPriceKey)(
    "returns ETH price for native token on Ethereum",
    async () => {
      const res = await GET(
        makeRequest({
  ...
  it.skipIf(!hasTokenPriceKey)(
    "returns USDC price on Ethereum",
    async () => {
      const res = await GET(
