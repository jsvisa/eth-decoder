...
const TRANSFER_TOPIC1 = '0x000000000000000000000000a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48'
const TRANSFER_TOPIC2 = '0x000000000000000000000000c02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
const TRANSFER_DATA   = '0x00000000000000000000000000000000000000000000000000000000000f4240'
...
const OPENCHAIN_TRANSFER_EVENT_RESPONSE = {
  ...
  result: {
    event: {
      [TRANSFER_TOPIC0]: [{ name: 'Transfer(address,address,uint256)', filtered: false }],
    },
    function: {},
  },
}
...
describe('GET /api/decode-event — basics', () => {
  ...
  it('returns backend result when backend succeeds', async () => {
    ...
    const res = await GET(makeRequest({ sign: TRANSFER_TOPIC0, topics: ALL_TOPICS, data: TRANSFER_DATA }))
...
describe('GET /api/decode-event — OpenChain fallback', () => {
  it('returns decoded event from OpenChain when backend returns non-OK', async () => {
    ...
    global.fetch.mockResolvedValueOnce({ ok: true, json: async () => OPENCHAIN_TRANSFER_EVENT_RESPONSE })
  ...
  it('returns decoded event from OpenChain when backend returns msg !== ok', async () => {
    process.env.BACKEND_URL = 'https://backend.test'
    global.fetch.mockResolvedValueOnce({ ok: true, json: async () => ({ msg: 'not found' }) })
    global.fetch.mockResolvedValueOnce({ ok: true, json: async () => OPENCHAIN_TRANSFER_EVENT_RESPONSE })
  ...
  it('returns original backend response when OpenChain also has no match', async () => {
    ...
    global.fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ ok: true, result: { event: { [TRANSFER_TOPIC0]: [] }, function: {} } }),
    })
