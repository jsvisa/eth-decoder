...
describe('GET /api/v1/decode', () => {
  ...
  it('returns 500 when BACKEND_URL is not set', async () => {
    const res = await decodeGET(makeRequest('/api/v1/decode', { data: '0x12345678' }))
  ...
  it('proxies to BACKEND_URL and returns the decoded result', async () => {
    ...
    const res = await decodeGET(makeRequest('/api/v1/decode', { data: '0xa9059cbb' }))
...
describe('GET /api/v1/decode-event', () => {
  ...
  it('returns 500 when BACKEND_URL is not set', async () => {
    ...
    const res = await decodeEventGET(makeRequest('/api/v1/decode-event', { sign: TOPIC0 }))
    expect(res.status).toBe(500)
    const body = await res.json()
...
describe('GET /api/v1/fetch-abi', () => {
  it('returns 400 when address param is missing', async () => {
    const res = await fetchAbiGET(makeRequest('/api/v1/fetch-abi', { apiKey: 'test' }))
    expect(res.status).toBe(400)
    const body = await res.json()
  ...
  it('returns 400 when no API key is provided', async () => {
    const res = await fetchAbiGET(makeRequest('/api/v1/fetch-abi', {
  ...
  it('returns the ABI for a verified contract', async () => {
    global.fetch.mockResolvedValueOnce({ ok: true, json: async () => etherscanErc20 })
